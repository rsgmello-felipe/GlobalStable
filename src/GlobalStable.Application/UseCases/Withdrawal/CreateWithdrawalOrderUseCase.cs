    ...
    ServiceDbContext dbContext)
{
        ...
        string? originHeader)
    {
        ...
        try
        {
            ...
            var (feeAmount, totalAmount, feeConfig) = feeResult.Value;
    ...
    private async Task<Result<(decimal FeeAmount, decimal TotalAmount, FeeConfig FeeConfig)>> GetFeeAndAmountsAsync(
        Domain.Entities.Account account,
        decimal amount)
    {
        var feeConfig = await feeConfigRepository.GetByAccountIdAsync(account.Id, TransactionOrderType.Withdrawal);
        if (feeConfig is null)
        {
            logger.LogCritical("Fee config not found for AccountId: {AccountId}", account.Id);
            return Result.Fail<(decimal, decimal, FeeConfig)>($"Fee config not found for AccountId: {account.Id}");
        }

        var feeAmount = (amount * feeConfig.FeePercentage) + feeConfig.FlatFee;
        var totalAmount = Math.Round(amount + feeAmount, account.Currency.Precision);

        return Result.Ok((feeAmount, totalAmount, feeConfig));
    }
